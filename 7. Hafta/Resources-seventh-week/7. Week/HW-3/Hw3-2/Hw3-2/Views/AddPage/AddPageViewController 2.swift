...
class AddPageViewController: UIViewController {
    ...
    @IBOutlet private weak var zookeeperNameTF: UITextField!
    @IBOutlet private weak var saveZookeeperOutlet: UIButton!
    @IBOutlet private weak var selectAnimalTypeOutlet: UIButton!
    @IBOutlet private var animalsCollection: [UIButton]!
    @IBOutlet private weak var animalNameTF: UITextField!
    @IBOutlet private weak var addAnimalView: UIStackView!
    @IBOutlet private weak var saveAnimalOutlet: UIButton!
    @IBOutlet private weak var selectZookeeperOutlet: UIButton!
    ...
    override func viewDidLoad() {
        ...
        print("Zookeeper Count: \(ZooModel.shared.allZookeeperList.count)")
    }
    
    //Animasyon bucalamaları. Görmezden gelin :D Şansımı deniyorum mantığını kavramaya çalışıyorum
    @IBAction private func selectZookeeper(_ sender: UIButton) {
        if ZooModel.shared.allZookeeperList.isEmpty {
    ...
    @IBAction private func selectAnimalType(_ sender: UIButton) {
        animalsCollection.forEach {
            btn in
            UIView.animate(withDuration: 0.8) {
    ...
    @IBAction private func animalCollection(_ sender: UIButton) {
    ...
    @IBAction private func saveZookeper(_ sender: Any) {
        ...
        alert.addAction(UIAlertAction(title: "Save", style: UIAlertAction.Style.default) {
            UIAlertAction in
            ZooModel.shared.allZookeeperList.append(newZookeeper)
    ...
    @IBAction private func saveAnimal(_ sender: Any) {
        ...
        for val in ZooModel.shared.allZookeeperList {
        ...
        alert.addAction(UIAlertAction(title: "Save", style: UIAlertAction.Style.default) {
            UIAlertAction in
            ZooModel.shared.waterLimit! -= newAnimal.waterConsumption
            ZooModel.shared.allAnimalList.append(newAnimal)
            self.navigationController?.popViewController(animated: true)
        })
        // show the alert
        self.present(alert, animated: true, completion: nil)
...
extension AddPageViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let name = ZooModel.shared.allZookeeperList[indexPath.row].name
...
extension AddPageViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.textLabel?.text = ZooModel.shared.allZookeeperList[indexPath.row].name
        return cell
    }
}
